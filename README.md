...
### What are BLS signatures?
    ...
    Boneh-Lynn-Shacham [i](https://en.wikipedia.org/wiki/Cryptography)s a cryptographic signature that allows a user to verify that a signer is authentic. It relies on elliptic curve pairings and is used by Ethereum due to its aggregation properties.
    
### How does Aligned work?
    
    The flow for fast verification is as follows:
    
    1. The user sends the proof and additional data to a verifier task batcher and waits for the network confirmation, receiving all the data needed to use the proof in Ethereum.
    2. The verifier task batcher posts the batch task in the service manager in Ethereum.
    3. The operators are notified of the task event, receive the proof and other data, and start the verification process.
    4. The operators sign the results and send the signatures to the aggregator.
    5. If more than two-thirds of the validators agree, the aggregator adds the signatures and sends them to the service manager in Ethereum.
    6. Ethereum checks the BLS signature, ensuring the task's quorum has been met. The results are stored in the smart contract, which the user will consume.
    7. The user receives the data needed to query the verification result through Aligned SDK and uses it in whichever way he needs!
### What is restaking?
...
### Can you provide an estimate of Aligned’s savings?
    ...
    - Batching 1024 proofs: 2,100 gas (includes reading and creating task)
