...
func TestFibonacciJoltProofVerifies(t *testing.T) {
	...
	commitmentFile, err := os.Open("../../task_sender/test_examples/jolt/fib_e2e/jolt.commitments")
	if err != nil {
		t.Errorf("could not open proof file: %s", err)
	}
...
func TestSha3JoltProofVerifies(t *testing.T) {
	...
	commitmentFile, err := os.Open("../../task_sender/test_examples/jolt/sha3_e2e/jolt.commitments")
	if err != nil {
		t.Errorf("could not open proof file: %s", err)
	}
