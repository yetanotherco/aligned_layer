...
func TestFibonacciJoltProofVerifies(t *testing.T) {
	proofFile, err := os.Open("../../scripts/test_files/jolt/fibonacci/fibonacci-guest.proof")
	if err != nil {
	...
	elfFile, err := os.Open("../../scripts/test_files/jolt/fibonacci/fibonacci-guest.elf")
	if err != nil {
		t.Errorf("could not open proof file: %s", err)
	}
...
func TestSha3JoltProofVerifies(t *testing.T) {
	proofFile, err := os.Open("../../scripts/test_files/jolt/sha3-ex/sha3-guest.proof")
	if err != nil {
		t.Errorf("could not open proof file: %s", err)
	}
	proofBytes := make([]byte, MaxProofSize)
	...
	elfFile, err := os.Open("../../scripts/test_files/jolt/sha3-ex/elf/sha3-guest.elf")
	if err != nil {
		t.Errorf("could not open proof file: %s", err)
	}
