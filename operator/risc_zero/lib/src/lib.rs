...
mod tests {
    ...
    fn read_image_id_from_file(file_name: &str) -> Result<[u32; 8], Box<dyn std::error::Error>> {
        ...
        // Remover los corchetes y los espacios en blanco adicionales
        let contents = contents.trim_matches(|c: char| c == '[' || c == ']' || c.is_whitespace());
        let parts: Vec<&str> = contents.split(',').collect();
        if parts.len() != 8 {
            return Err("El archivo no contiene exactamente 8 números".into());
        }
        let mut image_id = [0u32; 8];

        for (i, part) in parts.iter().enumerate() {
            image_id[i] = part.trim().parse().map_err(|e| format!("Error al parsear el número en la posición {}: {}", i, e))?;
        }
    ...
    fn verify_risc_zero_receipt_with_image_id_works() {
        ...
        // Leer el IMAGE_ID desde el archivo
