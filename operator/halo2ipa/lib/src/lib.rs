...
mod tests {
    ...
    const PROOF: &[u8] = include_bytes!("../../../../task_sender/test_examples/halo2_ipa/proof.bin");

    const PROOF_FILE_PATH: &str = "../../../task_sender/test_examples/halo2_ipa/proof.bin";

    const PUB_INPUT: &[u8] = include_bytes!("../../../../task_sender/test_examples/halo2_ipa/pub_input.bin");

    const PUB_INPUT_PATH: &str = "../../../task_sender/test_examples/halo2_ipa/pub_input.bin";

    const PARAMS: &[u8] = include_bytes!("../../../../task_sender/test_examples/halo2_ipa/params.bin");

    const PARAMS_FILE_PATH: &str = "../../../task_sender/test_examples/halo2_ipa/params.bin";
	...
	fn verify_halo2_plonk_proof() {
        ...
        let result = verify_halo2_ipa_proof_ffi(&proof_buffer, proof_len as u64, &cs_buffer, cs_len as u64, &vk_buffer, vk_len as u64, &ipa_params_buffer, ipa_params_len as u64, &public_input_buffer, public_input_len as u64);
        assert!(result)
	}
	...
	fn verify_halo2_plonk_proof_aborts_with_bad_proof() {
        ...
        let result = verify_halo2_ipa_proof_ffi(&proof_buffer, (proof_len - 1) as u64, &cs_buffer, cs_len as u64, &vk_buffer, vk_len as u64, &ipa_params_buffer, ipa_params_len as u64, &public_input_buffer, public_input_len as u64);
        assert!(!result)
	}
}