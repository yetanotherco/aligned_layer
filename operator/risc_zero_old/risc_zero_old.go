...
/*
#cgo linux LDFLAGS: ${SRCDIR}/lib/librisc_zero_verifier_old.so -ldl -lrt -lm -lssl -lcrypto -Wl,--allow-multiple-definition
...
func VerifyRiscZeroReceiptOld(innerReceiptBuffer []byte, imageIdBuffer []byte, publicInputBuffer []byte) bool {
	...
	return (bool)(C.verify_risc_zero_receipt_old_ffi(receiptPtr, (C.uint32_t)(len(innerReceiptBuffer)), imageIdPtr, (C.uint32_t)(len(imageIdBuffer)), publicInputPtr, (C.uint32_t)(len(publicInputBuffer))))
}
