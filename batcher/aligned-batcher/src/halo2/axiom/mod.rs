...
pub fn verify_halo2_axiom(proof: &[u8], public_input: &[u8], verification_key: &[u8]) -> bool {
    ...
    kzg_params_buffer[..kzg_params_len].clone_from_slice(&verification_key[kzg_offset..(kzg_offset + kzg_params_len)]);

    if let Ok(cs) = bincode::deserialize(&cs_buffer[..(cs_len as usize)]) {
        if let Ok(vk) = VerifyingKey::read_cs(
            ...
            if let Ok(params) = Params::read::<_>(&mut BufReader::new(
                &kzg_params_buffer[..kzg_params_len],
            )) {
