...
// to set the respondToTaskFeeLimit variable higher than fee_for_aggregator
const RESPOND_TO_TASK_FEE_LIMIT_MULTIPLIER: u128 = 5; 
const RESPOND_TO_TASK_FEE_LIMIT_DIVIDER: u128 = 2;
// to set the feeForAggregator variable higher than what was calculated
const DEFAULT_AGGREGATOR_FEE_MULTIPLIER: u128 = 3;
...
impl Batcher {
    ...
    async fn submit_batch(
        ...
        let fee_per_proof = U256::from(gas_per_proof) * gas_price;
        let fee_for_aggregator = (U256::from(AGGREGATOR_GAS_COST) * gas_price * U256::from(DEFAULT_AGGREGATOR_FEE_MULTIPLIER)) / U256::from(DEFAULT_AGGREGATOR_FEE_DIVIDER);
        let respond_to_task_fee_limit =
            (fee_for_aggregator * U256::from(RESPOND_TO_TASK_FEE_LIMIT_MULTIPLIER)) / U256::from(RESPOND_TO_TASK_FEE_LIMIT_DIVIDER);
